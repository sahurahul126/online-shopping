...
<body>
  <div class="container-fluid">
    ...
    <h3 class="text-center text-light bg-info p-2"> Mobile Filter</h3>
    </div>
    	...
    	<div class="col-lg-3">
    		...
    		<ul class="list-group"> 
    			<?php
    			$sql="SELECT DISTINCT brand FROM mobiles ORDER BY brand";
    		...
    		<ul class="list-group"> 
    			<?php
    			$sql="SELECT DISTINCT ram FROM mobiles ORDER BY ram";
    			$result=$conn->query($sql);
    			 while($row=$result->fetch_assoc()){
    		...
    		<ul class="list-group"> 
    			<?php
    			$sql="SELECT DISTINCT hdd FROM mobiles ORDER BY hdd";
    			$result=$conn->query($sql);
    			 while($row=$result->fetch_assoc()){
    		...
    		<ul class="list-group"> 
    			<?php
    			$sql="SELECT DISTINCT processor FROM mobiles ORDER BY processor";
    			$result=$conn->query($sql);
    			 while($row=$result->fetch_assoc()){
    		...
    		<ul class="list-group"> 
    			<?php
    			$sql="SELECT DISTINCT screen_size FROM mobiles ORDER BY screen_size";
    			$result=$conn->query($sql);
    			 while($row=$result->fetch_assoc()){
    		...
    		<ul class="list-group"> 
    			<?php
    			$sql="SELECT DISTINCT os FROM mobiles ORDER BY os";
    			$result=$conn->query($sql);
    			 while($row=$result->fetch_assoc()){
           			...
           			<div class="col-md-3 mb-2"> 
           				<div class="card-deck">
           					<div class="card border-secondary">
           						...
           						<div class="card-body">
           							<h4 class="card-title text-danger">Price:<?=($row['product_price']);?>/-
                        </h4>
           							<p>
           								RAM:<?=$row['ram'];?><br>
           								HDD:<?=$row['hdd'];?><br>
    ...
    <!-- <div class="row">
      ...
      if (isset($_GET['pageno'])) {
        ...
        $total_pages_sql = "SELECT COUNT(id) FROM mobiles";
        $result = mysqli_query($conn,$total_pages_sql);
        $total_rows = mysqli_fetch_array($result)[0];
        $total_pages = ceil($total_rows / $no_of_records_per_page);

        $sql = "SELECT * FROM mobiles LIMIT $offset, $no_of_records_per_page";
